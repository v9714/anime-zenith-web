...
export function MangaHeroLocal({ manga, autoPlay = true, interval = 6000 }: MangaHeroLocalProps) {
  ...
  const genres = current.genres?.slice(0, 4).map(g => g.genre.name) || [];
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-2xl space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-300 animate-fade-in">
              <span>{current.author || 'Unknown Author'}</span>
              {current.releaseYear && (
                <>
              ...
              {current.artist && (
                <>
                  <span className="w-1 h-1 rounded-full bg-gray-500" />
                  <span>Art by {current.artist}</span>
                </>
              )}
            </div>
