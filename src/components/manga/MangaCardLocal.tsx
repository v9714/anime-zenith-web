...
export function MangaCardLocal({ manga, variant = 'default', showStatus = true, className, style }: MangaCardLocalProps) {
  ...
  const genres = manga.genres?.slice(0, 3).map(g => g.genre.name) || [];
    ...
    >
      ...
      <div className="p-3 space-y-1 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          {manga.releaseYear && (
            <span className="flex-shrink-0 ml-2">{manga.releaseYear}</span>
          )}
        </div>
      </div>
    </Link>
